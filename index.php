...
?><div id="conteudo" class="modulos"><?php
...
while($array = $query->resultado()){
	if(isset($_SESSION['SISTEMAS'][$array['CODSIS']])){
		if($_SESSION['SISTEMAS'][$array['CODSIS']]['HABILITADO']){
			...
			$sistemas .= '<div id="sis'.$array['CODSIS'].'" ><a href="'.$array['PASTA'].'" ><img src="imagens/'.str_replace(' ','_',$array['NOMSIS']).'.png" alt="'.htmlentities(ucfirst(mb_strtolower($array['NOMSIS']))).'" title="'.htmlentities(ucfirst(mb_strtolower($array['NOMSIS']))).'" /></a></div>';
		} else {
			...
			//$sistemas .= '<div id="sis'.$array['CODSIS'].'" ><form name="form_sis'.$array['CODSIS'].'" onsubmit=" return autorizarModulo(this); " ><input type="hidden" name="pasta" value="'.$array['PASTA'].'" /><input type="hidden" name="codsis" value="'.$array['CODSIS'].'" /><input type="hidden" name="nomsis" value="'.$array['NOMSIS'].'" /><input type="hidden" name="chave" value="'.$_SESSION['SISTEMAS'][$array['CODSIS']]['SENHA'].'" /><img src="imagens/'.str_replace(' ','_',$array['NOMSIS']).'.png" alt="'.htmlentities(ucfirst(mb_strtolower($array['NOMSIS']))).'" title="'.htmlentities(ucfirst(mb_strtolower($array['NOMSIS']))).'" /><fieldset><input type="password" name="senha" value="" placeholder="Senha" /><input type="submit" value="Enviar" /></fieldset></form></div>';
		}
	}
}
